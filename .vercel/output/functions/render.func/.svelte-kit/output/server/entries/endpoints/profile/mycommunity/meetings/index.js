...
const GET = async ({ locals, request }) => withApiAuth({
  ...
  const { data: profileData, error } = await supabaseServerClient(request).from("profile").select("community_meeting_choices,other_community_meeting").eq("id", locals.user.id);
