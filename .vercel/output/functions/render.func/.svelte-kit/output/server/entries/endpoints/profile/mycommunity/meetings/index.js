...
const GET = async ({ locals }) => withApiAuth({
  ...
  const { data: profileData, error } = await supabaseServerClient(locals.accessToken).from("profile").select("community_meeting_choices,other_community_meeting").eq("id", locals.user.id);
